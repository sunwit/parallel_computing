...
int main()
{
  ...
  struct work_entry *data = (struct work_entry*) malloc(10000);  // read data from the file
  ...
  memSize = dimSize*sizeof(struct work_entry);
  ...
  for(int j=0;j<dimSize;j++)  //input data to the host memory 
  {
    h_a[j].cmd = data[j].cmd;
    h_a[j].pkey = data[j].pkey;
  }
  ...
  transformerPro<<<dimGrid, dimBlock>>>(d_a, dimSize); // the most time conumming part in producer side
  ...
  transformerCon<<<dimGrid, dimBlock>>>(d_a, dimSize); //the most time consumming part in consumer side 
               
  cudaMemcpy(h_a, d_a, memSize, cudaMemcpyDeviceToHost); //copy back the data from the device to host 
  for(int i=0;i<dimSize;i++) //consumer prints the result 
  {
    int idx = i % BUF_SIZE;
