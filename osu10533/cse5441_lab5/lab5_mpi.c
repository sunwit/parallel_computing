...
if(rank==0)  //master process
{
  ...
  for(int i=0;fgets(lines, sizeof(lines), stdin);i++) // reading the data from file 
  {
    ...
    if(*token=='A' || *token=='B' || *token=='C' || *token=='D')
    {
       ...
       {
           if(cm=='A') { data[num].cmd = 'A'; data[num].key = atoi(token);data[num].nums=0;data[num].alpha=0;data[num].pWallTime=0;data[num].cWallTime=0;}
           if(cm=='B') { data[num].cmd = 'B'; data[num].key = atoi(token);data[num].nums=0;data[num].alpha=0;data[num].pWallTime=0;data[num].cWallTime=0;}
           if(cm=='C') { data[num].cmd = 'C'; data[num].key = atoi(token);data[num].nums=0;data[num].alpha=0;data[num].pWallTime=0;data[num].cWallTime=0;}
           if(cm=='D') { data[num].cmd = 'D'; data[num].key = atoi(token);data[num].nums=0;data[num].alpha=0;data[num].pWallTime=0;data[num].cWallTime=0;}
           num = num + 1;
       }
    }
  }
